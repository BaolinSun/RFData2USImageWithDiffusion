...
from util import save_image, zero_grad
...
def main():
    ...
    model_params = list(chain(model.parameters(),autoencoder.parameters()))
    optimizer = torch.optim.AdamW(params=model_params, lr = args.lr, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            valid = torch.Tensor(np.ones((args.batch_size, *patch))).to(device)
            fake = torch.Tensor(np.zeros((args.batch_size, *patch))).to(device)

            pred_fake = discriminator(encoder_rf, losses['model_output'])
            loss_gan = criterion_GAN(pred_fake, valid)

            loss_pixel = criterion_pixelwise(losses["model_output"], us_image)

            loss = loss_unet + loss_ae + loss_gan + loss_pixel
            ...
            # Fake loss
            pred_fake = discriminator(encoder_rf.detach(), losses["model_output"].detach())
            ...
            writer.add_scalar(tag='loss/gan', scalar_value=loss_gan, global_step=g_step)
            writer.add_scalar(tag='loss/pixel', scalar_value=loss_pixel, global_step=g_step)
            writer.add_scalar(tag='loss/real_loss', scalar_value=loss_real, global_step=g_step)
            writer.add_scalar(tag='loss/fake_loss', scalar_value=loss_fake, global_step=g_step)

            if (g_step + 1) % args.log_interval == 0:
                logger.info(f"epoch: {epoch}, step: {g_step+1}, loss: {loss.item()}, loss_unet: {loss_unet.item()}, loss_ae: {loss_ae.item()}, gan: {loss_gan.item()}, pixel: {loss_pixel.item()}, real_loss: {loss_real.item()}, fake_loss: {loss_fake.item()}")
